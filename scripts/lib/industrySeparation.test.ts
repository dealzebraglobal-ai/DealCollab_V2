...
console.log('industrySeparation');
...
test('cosine: magnitude-invariant', () => {
    assert.equal(Math.round(cosine([1, 2], [10, 20]) * 1e6) / 1e6, 1);
});

test('cosine: rejects mismatched lengths', () => {
    assert.throws(() => cosine([1, 2], [1, 2, 3]), /length mismatch/);
});

test('clean gap -> separable, threshold lands inside the gap', () => {
...
test('overlapping clusters -> NOT separable, names the offenders', () => {
    const r = findBestThreshold([
...
test('UNDECIDED pairs are excluded from fitting', () => {
    const r = findBestThreshold([
...
test('one-sided label set -> not separable, threshold null', () => {
    const r = findBestThreshold([pair('ACCEPT', 0.9), pair('ACCEPT', 0.8)]);
    assert.equal(r.separable, false);
    assert.equal(r.threshold, null);
});

test('boundary: equal scores across classes cannot separate', () => {
...
test('ground-truth set is well formed', () => {
